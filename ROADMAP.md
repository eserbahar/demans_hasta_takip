...
- **Görsel tasarım**: Sıcak krem arka plan (`#F6F2EA`), koyu ada yeşili vurgu rengi (`#2F6F5E`), Fraunces (başlık) + Figtree (gövde) yazı tipleri, her kategori için ayrı bir vurgu rengi. Tasarım önce bir HTML mockup (Artifact) üzerinde kullanıcıyla birlikte defalarca yinelenip onaylandı, sonra gerçek Flutter koduna geçirildi. Tam renk paleti, kategori/ikon eşlemesi ve mockup linki için bkz. [DESIGN.md](DESIGN.md).
...
- **Yayına alma sonrası düzeltmeler**: İlk redesign push'undan sonra GitHub Pages'te canlı test edilince iki hata bulundu ve ayrı bir commit'te düzeltildi — AppBar'ların beyaz kalması (tema eksikliği) ve kategori grid'inin bazı ekran boyutlarında sığmaması (`childAspectRatio` yerine sabit `mainAxisExtent`'e geçildi). Detaylar [HISTORY.md](HISTORY.md)'de.
